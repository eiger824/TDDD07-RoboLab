...
// Split list in two lists of half the sizes
doublylinkedlist_node_t *split(doublylinkedlist_node_t *head)
{
    doublylinkedlist_node_t *fast = head;
    doublylinkedlist_node_t *slow = head;
    while(fast->next && fast->next->next)
    {
        fast = fast->next->next;
        slow = slow->next;
    }
    doublylinkedlist_node_t *tmp = slow->next;
    slow->next = NULL;
    return tmp;
}

// Merge list halves
doublylinkedlist_node_t *merge(doublylinkedlist_node_t *first,
        doublylinkedlist_node_t *second)
{
    // If first half list is empty
    if (!first)
    {
        return second;
    }
    // If second half list is empty
    if (!second)
    {
        return first;
    }
    doublylinkedlist_node_t *res;
    // Sort by ascending order i.e. by critical level
...
// sorting algorithm
doublylinkedlist_node_t *mergeSort(doublylinkedlist_node_t *head)
{
    if (!head || !head->next)
    {
        return head;
    ...
    doublylinkedlist_node_t *second = split(head);

    // Merge sort them separately
    head = mergeSort(head);
    second = mergeSort(second);

    // return the result of merging both semilists
    return merge(head, second);
}

// Function wrapper to be called from task_communicate
doublylinkedlist_t* doublylinkedlist_sort_by_critical(doublylinkedlist_t *dlls)
{
    dlls->first = mergeSort(dlls->first);
    return dlls;
}
